...
<div class="page-header">
    <h2>Detail Laporan Ditugaskan</h2>
    <a href="<?= base_url('/petugas/reports') ?>" class="btn btn-secondary">Kembali</a>
</div>
...
<div class="panel mb-20">
    ...
    <?php if ($report['status'] === 'diverifikasi') : ?>
        <div class="form-card mb-20">
            <h3>Mulai Proses Laporan</h3>
            <p>
                Klik tombol di bawah ini jika laporan sudah mulai ditangani.
                Status laporan akan berubah menjadi <strong>Diproses</strong>.
            </p>

            <form action="<?= base_url('/petugas/reports/' . $report['id'] . '/process') ?>" method="post">
                <?= csrf_field() ?>

                <button type="submit" class="btn btn-warning" onclick="return confirm('Ubah status laporan menjadi diproses?')">
                    Ubah Menjadi Diproses
                </button>
            </form>
        </div>
    <?php endif; ?>

    <?php if ($report['status'] === 'diproses') : ?>
        <div class="form-card mb-20">
            <h3>Selesaikan Laporan</h3>

            <form action="<?= base_url('/petugas/reports/' . $report['id'] . '/complete') ?>" method="post">
                <?= csrf_field() ?>

                <div class="form-group">
                    <label for="completed_note">Catatan Penyelesaian</label>
                    <textarea
                        name="completed_note"
                        id="completed_note"
                        class="form-control"
                        placeholder="Contoh: Lampu ruang kelas sudah diganti dan fasilitas kembali normal."
                        required
                    ><?= old('completed_note') ?></textarea>
                </div>

                <button type="submit" class="btn btn-success" onclick="return confirm('Selesaikan laporan ini?')">
                    Tandai Selesai
                </button>
            </form>
        </div>
    ...
    <?php if (! in_array($report['status'], ['selesai', 'ditolak'])) : ?>
        <div class="form-card">
            <h3>Tambah Komentar / Tanggapan</h3>

            <form action="<?= base_url('/petugas/reports/' . $report['id'] . '/comment') ?>" method="post">
                <?= csrf_field() ?>
                ...
                <div class="form-group">
                    ...
                    <textarea
                        ...
                        id="comment"
                        class="form-control"
                        placeholder="Tulis tanggapan atau perkembangan penanganan laporan."
                        required
