...
<div class="panel mb-20">
    ...
    <div class="quick-actions mt-20">
        <?php if (in_array($report['status'], ['menunggu_verifikasi', 'ditolak'])) : ?>
            <a href="<?= base_url('/laporan/' . $report['id'] . '/edit') ?>" class="btn btn-warning">
                Edit Laporan
            </a>
        <?php endif; ?>

        <?php if (in_array($report['status'], ['diverifikasi', 'diproses', 'selesai'])) : ?>
            <form action="<?= base_url('/laporan/' . $report['id'] . '/vote') ?>" method="post">
                <?= csrf_field() ?>
                <button type="submit" class="btn btn-success">Dukung Laporan</button>
            </form>
        <?php endif; ?>
    </div>
</div>
...
<div class="panel">
    ...
    <?php else : ?>
        <div class="empty-state">Belum ada komentar atau tanggapan.</div>
    <?php endif; ?>
</div>
